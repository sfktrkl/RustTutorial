...
// features that came from functional programming. Arguably, OOP languages share
// certain common characteristics, namely objects, encapsulation, and
// inheritance. Let’s look at what each of those characteristics means and
...
// private. For example, we can define a struct AveragedCollection that has a
// field containing a vector of i32 values. The struct can also have a field
// that contains the average of the values in the vector, meaning the average
// doesn’t have to be computed on demand whenever anyone needs it. In other
// words, AveragedCollection will cache the calculated average for us.
...
fn main() {
    ...
    // added to list using the add method or removed using the remove method,
    // the implementations of each call the private update_average method that
    ...
    // AveragedCollection wouldn’t need to change. If we made list public
    // instead, this wouldn’t necessarily be the case: HashSet<i32> and Vec<i32>
    ...
    // or not for different parts of code enables encapsulation of
    ...
    // the parent struct’s fields and method implementations. However, if you’re
    // used to having inheritance in your programming toolbox, you can use other
    // solutions in Rust, depending on your reason for reaching for inheritance
    // in the first place.
    ...
    // share Rust code using default trait method implementations instead, which
    // you saw in Listing 10-14 when we added a default implementation of the
    // summarize method on the Summary trait. Any type implementing the Summary
    // trait would have the summarize method available on it without any further
    // code. This is similar to a parent class having an implementation of a
    // method and an inheriting child class also having the implementation of
    // the method. We can also override the default implementation of the
    // summarize method when we implement the Summary trait, which is similar to
    // a child class overriding the implementation of a method inherited from a
    // parent class.

    // The other reason to use inheritance relates to the type system: to enable
    // a child type to be used in the same places as the parent type. This is
    // also called polymorphism, which means that you can substitute multiple
