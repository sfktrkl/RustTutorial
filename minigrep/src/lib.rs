...
// We added code that took a slice of String values and created an instance of
// the Config struct by indexing into the slice and cloning the values, allowing
// the Config struct to own those values.
// At the time, we said not to worry about the inefficient clone calls because
// we would remove them in the future. Well, that time is now!
// We needed clone here because we have a slice with String elements in the
// parameter args, but the new function doesn’t own args. To return ownership
// of a Config instance, we had to clone the values from the query and filename
// fields of Config so the Config instance can own its values.
// With our new knowledge about iterators, we can change the new function to
// take ownership of an iterator as its argument instead of borrowing a slice.
// We’ll use the iterator functionality instead of the code that checks the
// length of the slice and indexes into specific locations. This will clarify
// what the Config::new function is doing because the iterator will access the
// values.
...
// into Config rather than calling clone and making a new allocation.
...
impl Config3 {
    ...
    // the type of the iterator it returns is std::env::Args. We’ve updated the
    ...
    pub fn new(mut args: env::Args) -> Result<Config3, &'static str> {
        // Using Iterator Trait Methods Instead of Indexing
        // Next, we’ll fix the body of Config::new. The standard library
        // documentation also mentions that std::env::Args implements the
        // Iterator trait, so we know we can call the next method on it!
        // Remember that the first value in the return value of env::args is the
        // name of the program. We want to ignore that and get to the next value,
        // so first we call next and do nothing with the return value. Second,
...
pub fn search2<'a>(query: &str, contents: &'a str) -> Vec<&'a str> {
    ...
    // contents that contain the query. This code uses the filter adaptor to
    // keep only the lines that line.contains(query) returns true for. We then
    // collect the matching lines into another vector with collect. Much simpler!
    contents
        .lines()
...
pub fn search_case_insensitive2<'a>(query: &str, contents: &'a str) -> Vec<&'a str> {
    contents
        .lines()
...
// focuses on the high-level objective of the loop. This abstracts away some of
// the commonplace code so it’s easier to see the concepts that are unique to
// this code, such as the filtering condition each element in the iterator must pass.
pub fn run5(config: Config3) -> Result<(), Box<dyn Error>> {
    let contents = fs::read_to_string(config.filename)?;
    let results = if config.case_sensitive {
        search2(&config.query, &contents)
    } else {
        search_case_insensitive2(&config.query, &contents)
    };
    for line in results {
        println!("{}", line);
    }
