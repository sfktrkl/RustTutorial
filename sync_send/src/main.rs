...
// is Send, but there are some exceptions, including Rc<T>: this cannot be Send
...
// same time. For this reason, Rc<T> is implemented for use in single-threaded
// situations where you don’t want to pay the thread-safe performance penalty.

// Therefore, Rust’s type system and trait bounds ensure that you can never
// accidentally send an Rc<T> value across threads unsafely. When we tried to do
// this, we got the error the trait Send is not implemented for Rc<Mutex<i32>>.
...
// Sync if &T (an immutable reference to T) is Send, meaning the reference can
// be sent safely to another thread. Similar to Send, primitive types are Sync,
